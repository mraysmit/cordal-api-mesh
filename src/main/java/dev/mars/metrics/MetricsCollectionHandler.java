...
public class MetricsCollectionHandler {
    ...
    private boolean shouldCollectMetrics(Context ctx) {
        if (!appConfig.getMetricsCollection().isEnabled()) {
            return false;
        }
        
        String path = ctx.path();
        List<String> excludePaths = appConfig.getMetricsCollection().getExcludePaths();
        
        return excludePaths.stream().noneMatch(path::startsWith);
    }
    
    private boolean shouldSampleRequest() {
        double samplingRate = appConfig.getMetricsCollection().getSamplingRate();
