 ...
 * Moved to common-library for reusability across modules
 */
public class DataLoader {
    ...
    public DataLoader(BaseDatabaseManager databaseManager, DataLoaderConfig config) {
    ...
    public void loadSampleDataIfNeeded() {
        if (!config.isSampleDataLoadingEnabled()) {
            logger.info("Sample data loading is disabled, skipping data loading");
            return;
        }
        
        logger.info("Loading sample data for database: {}", config.getDatabaseUrl());
    ...
    private void loadSampleData() throws SQLException {
        int sampleDataSize = config.getSampleDataSize();
